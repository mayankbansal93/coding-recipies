...
import com.mayank.freedom_from_nested_if_else.goodexample.executors.command_executor_impl.salary_command_executor_impl.FivePercentTaxWith300KLimitCommandExecutor;
...
public class SeniorCitizenCommandExecutor extends AgeCommandExecutor {
    @Override
    protected Boolean isValid(Age age) {
        return age.getAge() >= 60 && age.getAge() <= 80;
    }

    @Override
    protected TaxResponse executeValidCommand(Age age) {
        Salary salary = age.getSalary();
        List<CommandExecutor> commandExecutorList = ImmutableList.of(new FivePercentTaxWith300KLimitCommandExecutor(),
    ...
    public Boolean isApplicable(Request request) {
        return Constants.SENIORCITIZEN.equals(((Age) request).getCategory());
    }
}
